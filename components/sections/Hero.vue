<template>
  <section id="hero" class="min-h-screen flex flex-col px-6 md:px-16 lg:px-24">
    <!-- Navigation -->
    <nav class="flex flex-col items-center md:items-end gap-3 md:gap-0 pt-8 md:pt-10 animate-hero-1">
      <div class="flex justify-center md:justify-end items-center gap-4 sm:gap-6 md:gap-8 flex-wrap">
        <a
        ...
        <!-- Signature inline on desktop -->
        <div
          class="hidden md:block ml-6 w-[120px] h-[50px] -rotate-[1.5deg]"
          style="background-color: var(--text-muted); -webkit-mask-image: url('/images/signature.svg'); -webkit-mask-size: contain; -webkit-mask-repeat: no-repeat; mask-image: url('/images/signature.svg'); mask-size: contain; mask-repeat: no-repeat;"
        ></div>
      </div>

      <!-- Signature below nav on mobile -->
      <div
        class="md:hidden w-[110px] h-[45px] -rotate-[1.5deg] mt-1"
        style="background-color: var(--text-muted); -webkit-mask-image: url('/images/signature.svg'); -webkit-mask-size: contain; -webkit-mask-repeat: no-repeat; mask-image: url('/images/signature.svg'); mask-size: contain; mask-repeat: no-repeat;"
      ></div>
    </nav>
